  ...
  return (
    <div className="flex flex-col w-full items-center">
      ...
      {approvedNeeded && !isSwapping ? (
        <button disabled={!canApprove} onClick={onApproveRequested} className={`${canApprove ? "bg-purple-500 text-white" : "bg-site-dim2 text-site-dim2"} ${styles.actionButton}`}>
          {isApproving ? "Approving..." : "Approve"}
        </button>
      ) : (
        <button disabled={!canSwap} onClick={onSwapRequested} className={`${canSwap ? "bg-purple-500 text-white" : "bg-site-dim2 text-site-dim2"} ${styles.actionButton}`}>
          {isSwapping ? "Swapping..." : hasEnoughBalance ? "Swap" : "Insufficient balance"}
        </button>
      )}

      {failureMessage && !resetState ? <p className={styles.message}>{failureMessage}</p> : successMessage ? <p className={styles.message}>{successMessage}</p> : ""}
    </div>
  );
};
